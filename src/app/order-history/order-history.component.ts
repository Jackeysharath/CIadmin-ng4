...
export class OrderHistoryComponent implements OnInit {
  ...
  tokenCheck(){
    ...
    getHistory(){
      this._api.POST('GetFinalizedOrderHistory', {token: 'SomeTokenHere','patientid':this.user.uid,'mobileno':''}).subscribe(data =>{
        this.myFinalizedOrders=JSON.parse(data.json).data;
        
      if(this.myFinalizedOrders.length > 0){
  ...
  getBillDetails(bill_no){
   ...
   // console.log(bill_no);
    this.billDetails=[];
    this._api.POST('GetOrderDetails', {token: 'SomeTokenHere','orderno':bill_no,'mobileno':''}).subscribe(data =>{
  ...
  downloadReport(tid:any,bill_no:any){
    this._api.POST('GetFinalReport', {TokenNo: 'SomeTokenHere','service_id':tid,'orderno':bill_no}).subscribe(data =>{
